...
```
┌──────────────────────────────────┐
│ Hazakura Amp            ● 動作中 │ ← Header + StatusIndicator
│                                  │
...
> v0.2 では「UIでなくても機能するもの」を削り、Reset / preset / ON-OFF は表示しない。実装UI文字列は日本語が正（§8）。内部状態と診断コピー（クリップボード送信）は英語寄りで安定させる。
...
- このドキュメント内の英語UIテキストは **設計説明用の補足** に留め、**実装上の正本は日本語** として扱う
  ...
  - エラーバナー: 「⚠️ システム音声へのアクセスが許可されていません」（設計メモ: 英語UI案 = `System audio access is not allowed.`）
  - 起動時バナー: 「ブーストを開始してください / システム音をローカル処理します。録音・保存・送信は行いません。」（設計メモ: 英語UI案 = `Hazakura Amp will process system audio locally to apply a volume boost. Audio is not recorded, stored, or transmitted.`）
